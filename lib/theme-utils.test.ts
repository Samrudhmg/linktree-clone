...
describe("getThemeStyles()", () => {
  it("maps background.primary to --theme-bg-primary", () => {
    ...
    expect((styles as any)["--theme-bg-primary"]).toBe("#6366F1");
  });

  it("maps background.secondary to --theme-bg-secondary", () => {
    const styles = getThemeStyles(baseConfig);
    expect((styles as any)["--theme-bg-secondary"]).toBe("#A855F7");
  });

  it("maps text.primary to --theme-text-primary", () => {
    const styles = getThemeStyles(baseConfig);
    expect((styles as any)["--theme-text-primary"]).toBe("#ffffff");
  });

  it("maps text.secondary to --theme-text-secondary", () => {
    const styles = getThemeStyles(baseConfig);
    expect((styles as any)["--theme-text-secondary"]).toBe("#e5e7eb");
  });

  it("maps button.accent to --theme-accent", () => {
    const styles = getThemeStyles(baseConfig);
    expect((styles as any)["--theme-accent"]).toBe("#6366F1");
  });

  it("maps title styling properties correctly", () => {
    const styles = getThemeStyles(baseConfig);
    expect((styles as any)["--theme-title-color"]).toBe("#ffffff");
    expect((styles as any)["--theme-title-size"]).toBe("1.5rem");
    expect((styles as any)["--theme-title-weight"]).toBe("bold");
  });

  it("maps bio styling properties correctly", () => {
    const styles = getThemeStyles(baseConfig);
    expect((styles as any)["--theme-bio-color"]).toBe("#e5e7eb");
    expect((styles as any)["--theme-bio-size"]).toBe("1rem");
    expect((styles as any)["--theme-bio-weight"]).toBe("normal");
  });
  ...
  it("falls back to text.primary for --theme-title-color when title is missing", () => {
    ...
    expect((styles as any)["--theme-title-color"]).toBe("#ffffff"); // text.primary
  });
  ...
  it("falls back to text.secondary for --theme-bio-color when bio is missing", () => {
    ...
    const styles = getThemeStyles(config);
    expect((styles as any)["--theme-bio-color"]).toBe("#e5e7eb"); // text.secondary
  });
